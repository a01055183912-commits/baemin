...
const INTRO_MODE_SPEC = {
  brief: {
    ...
    defaultLen: 200,
    direction: '확인된 사실 중 가게 정체성·음식 특징·핵심 강점 중 중요한 것만 골라 짧고 빠르게 연결해서 설명',
    structureNote: '가게 정체성, 음식 특징, 핵심 강점 중 중요한 정보를 골라 자연스럽게 연결해주세요. 대표메뉴·가격을 포함할지는 아래 플랫폼별 작성 규칙을 우선 따르세요(규칙에 없으면 넣지 마세요). 긴 인사말이나 메뉴·가격의 기계적인 나열은 줄여주세요. 철학을 입력했다면 짧게 반영할 수 있습니다. 확인된 사실이 적으면 이보다 짧아도 괜찮습니다.',
  },
  detailed: {
    mode: 'detailed',
    label: '상세 소개',
    ...
    structureNote: '간단 소개에 같은 말을 반복해 붙이지 마세요. 가게 인사·정체성 → 사장님 철학 → 음식·재료·조리 특징 → 분위기 또는 자연스러운 마무리 순으로, 입력된 항목만 골라 3~4개 짧은 문단으로 써주세요. 철학이나 분위기가 비어 있으면 해당 부분은 생략하세요. 대표메뉴·가격을 포함할지는 아래 플랫폼별 작성 규칙을 우선 따르세요(규칙에 없으면 넣지 마세요). 확인된 사실이 적으면 권장 범위보다 짧아도 괜찮습니다. 분량을 채우려고 인기·단골·맛·건강 효과 등을 새로 만들지 마세요.',
  },
}
...
function introPlacementFor(platform, introMode) {
  ...
  const table = {
    '배민앱': {
      brief: { place: '가게 소개', rule: '가게 소개 영역에 게시합니다(입력 한도 최대 500자). 대표메뉴·가격은 메뉴 설명 영역에서 따로 다루니, 가게 소개에는 넣지 마세요. 간단 소개이니 가게 정체성과 맛의 특징, 핵심 강점 중 중요한 것만 짧고 빠르게 연결해주세요. 외부 주문·결제 유도, 외부 링크·SNS 홍보 문구는 넣지 마세요.' },
      detailed: { place: '가게 소개', rule: '가게 소개 영역에 게시합니다(입력 한도 최대 500자). 대표메뉴·가격은 메뉴 설명 영역에서 따로 다루니, 가게 소개에는 넣지 마세요. 상세 소개이니 가게 인사·정체성 → 사장님 철학 → 음식·재료·조리 특징 → 분위기 또는 자연스러운 마무리 순으로, 입력된 항목만 골라 충분히 소개해주세요. 외부 주문·결제 유도, 외부 링크·SNS 홍보 문구는 넣지 마세요.' },
    },
    '네이버 플레이스': {
...
const TEMPLATES = [
  ...
  { id: 1, title: '처음 오신 손님께 소개', category: '가게 소개·기본', type: '가게 소개', optionalPlatform: null, supportsIntroMode: true,
    ...
    example: '20년째 같은 자리에서 얼큰돼지국밥을 끓이는 집입니다. 매일 아침 육수를 직접 우려내고, 국내산 돼지고기만 사용합니다. 평일 점심엔 직장인 손님이 많고, 혼자 오셔도 편하게 드실 수 있어요.' },
  ...
  { id: 4, title: '배민 소개란 문구', category: '가게 소개·기본', type: '가게 소개', optionalPlatform: '배민앱', supportsIntroMode: true,
    ...
    example: '매일 아침 육수를 직접 끓이는 20년 전통 돼지국밥집입니다. 국내산 돼지고기만 사용하고, 평일 점심엔 직장인 손님이 많이 찾습니다.' },
  ...
  // E. 리뷰·고객 응대
  { id: 21, title: '정성스러운 좋은 리뷰 답글', category: '리뷰·고객 응대', type: '리뷰 답변', optionalPlatform: null,
    instruction: '별점 5점 리뷰 "[리뷰 내용]"에 답글을 써주세요. 손님이 남긴 칭찬이나 구체적인 표현 하나를 자연스럽게 짚어, 사장님이 직접 읽고 답하는 느낌으로요. 짧은 리뷰라도 형식적인 한두 문장으로 끝내지 말고 3~5문장 정도로요. 리뷰에 없는 사실을 새로 만들거나 가게 홍보 문구를 억지로 덧붙이지 마세요.', quickBlank: { key: 'reviewText', label: '실제 리뷰 내용 (별점 5점)', placeholder: '예: 국물이 깔끔해요' }, reviewMeta: { rating: '5' }, targetLen: null,
    example: '국물이 깔끔하게 느껴지셨다니 정말 반갑습니다. 짧은 한마디지만 맛있게 드셨다는 마음이 전해져 저희도 기분이 좋네요. 이렇게 직접 후기까지 남겨주셔서 감사합니다.' },
  { id: 22, title: '불편 리뷰 답글', category: '리뷰·고객 응대', type: '리뷰 답변', optionalPlatform: null,
    instruction: '별점 2점 리뷰 "[불편 내용]"에 답글을 써주세요. 손님이 불편하게 느낀 구체적인 내용을 먼저 짚고, 공감과 사과를 중심으로 쓰되 확인되지 않은 원인이나 책임을 임의로 단정하지 마세요. 확인·개선·보상·환불·연락 등 새로운 약속은 만들지 말고, 필요하면 "확인해보겠습니다" 정도까지만요. 재방문이나 재구매를 무리하게 권하지 말고, 3~5문장 정도로 사장님이 직접 답하는 것처럼 자연스럽게요.', quickBlank: { key: 'reviewText', label: '실제 리뷰 내용 (별점 2점, 불편)', placeholder: '예: 오늘따라 국물이 좀 짰어요' }, reviewMeta: { rating: '2', complaint: true }, targetLen: null,
    example: '국물이 짜게 느껴지셨다니 죄송합니다. 기대하고 주문하셨을 텐데 불편하게 드시게 한 것 같아 마음이 무겁습니다. 말씀해주신 내용은 확인해보겠습니다. 솔직한 의견 남겨주셔서 감사합니다.' },
  { id: 23, title: '배달 지연 리뷰 답글', category: '리뷰·고객 응대', type: '리뷰 답변', optionalPlatform: null,
    instruction: '배달이 늦어 불편했던 손님의 경험에 먼저 공감해주세요. 확인되지 않은 배달 지연 원인을 임의로 만들어내지 말고, 배달기사·배달 플랫폼·제3자의 책임이라고 단정하지 마세요. 변명보다는 공감과 사과를 우선하고, 사장님이 직접 읽고 답하는 것처럼 3~5문장 정도로요.', quickBlank: { key: 'reviewText', label: '실제 리뷰 내용 (배달 지연)', placeholder: '예: 배달이 너무 늦게 왔어요' }, reviewMeta: { complaint: true }, targetLen: null,
    example: '배달이 늦어 많이 기다리셨겠어요. 기대하고 기다리셨을 텐데 불편을 드려 죄송합니다. 말씀해주신 부분은 확인해보겠습니다. 다시 한번 죄송하고, 의견 남겨주셔서 감사합니다.' },
  { id: 24, title: '답글 표현 5가지 받아보기', category: '리뷰·고객 응대', type: '리뷰 답변', optionalPlatform: null,
    instruction: '같은 리뷰에 사용할 수 있는 답글을 5가지 버전으로 써주세요. 5개 모두 손님이 남긴 구체적인 표현에는 반응해야 합니다. 단순히 단어 몇 개만 바꾸지 말고 시작 문장·감사 표현·감정 표현·문장 구조·마무리 표현이 서로 다르게 느껴지도록 써주세요. 리뷰에 없는 가게 정보는 새로 추가하지 마세요.', quickBlank: { key: 'reviewText', label: '실제 리뷰 내용', placeholder: '최근에 받은 리뷰를 붙여넣어주세요' }, targetLen: null,
    example: '"국물 맛에 정성이 느껴진다는 말씀, 진심으로 감사합니다." (5가지 버전 중 한 예시)' },
  { id: 25, title: '손님의 칭찬을 살린 답글', category: '리뷰·고객 응대', type: '리뷰 답변', optionalPlatform: null,
    instruction: '손님이 남긴 구체적인 칭찬이나 경험을 답글 속에서 자연스럽게 한 번 짚어주세요. 다만 손님 한 명의 경험을 가게 전체의 객관적인 특징이나 모든 손님의 공통 평가처럼 일반화하지 마세요. 답글을 가게 광고로 바꾸지 말고, 손님의 경험에 대한 감사와 사장님의 자연스러운 반응을 중심으로 써주세요.', quickBlank: { key: 'reviewText', label: '실제 리뷰 내용 (칭찬)', placeholder: '예: 사장님이 친절하셔서 또 오고 싶어요' }, targetLen: null,
    example: '친절하다고 느껴주셨다니 저도 마음이 따뜻해지네요. 그렇게 말씀해주시니 오늘 하루 힘이 났습니다. 다음에 오시면 오늘처럼 편하게 맞이하겠습니다.' },
...
export function buildQuickDraft(profile, platformKey, introSpec) {
  ...
  const parts = []
  if (platformKey === '배민앱') {
    // 가게 소개: 대표메뉴·가격은 메뉴 설명에서 별도로 다루므로 여기서는 정체성·특징·강점만
    if (detailed) {
...
export function buildReviewReplyRequest(profile, task, opts) {
  ...
  out += `별점(참고용, 답글 본문에 직접 언급 금지): ${task.rating ? `${task.rating}점` : '미입력'}\n`
  ...
  out += `분량: ${opts.shorter ? '자연스러운 두 문장 정도로 짧게' : '짧은 리뷰라도 "감사합니다. 다음에 또 오세요" 정도의 한두 문장으로 끝내지 마세요. 가능하면 3~5문장 정도로, 손님의 리뷰를 실제로 읽었다는 느낌이 나게 써주세요. 일반적으로 150~300자 정도를 권장하지만, 리뷰가 짧으면 억지로 길게 만들지 말고 리뷰가 상세하면 그에 맞춰 조금 더 길게 써도 됩니다.'}${hasEventPromo ? ' (이벤트 안내 문장 포함하면 조금 더 길어져도 됨)' : ''}${platformLenNote}\n`
  ...
  out += `\n[꼭 지킬 원칙]\n`
  out += `- 손님이 실제로 남긴 표현이나 경험 중 하나 이상에 직접 반응하세요. 리뷰 문장을 그대로 복사해 반복하지 말고, 그 말에 대한 사장님의 자연스러운 감정과 반응을 덧붙이세요.\n`
  out += `- 별점은 답글 분위기를 정할 때만 참고하고, 완성된 답글 본문에는 별점을 직접 언급하지 마세요. 금지 예: "별점 5점 감사합니다", "5점까지 주셔서 감사합니다".\n`
  out += `- 매번 비슷한 감사 문구나 마무리를 반복하지 마세요. "소중한 리뷰 감사합니다", "기분이 좋네요", "힘이 납니다", "다음에도 찾아주세요", "더 맛있게 준비하겠습니다" 같은 표현은 필요하면 쓸 수 있지만, 매 리뷰마다 똑같이 쓰지 말고 리뷰 내용에 맞게 다양하게 바꿔주세요.\n`
  out += `- 답글 길이를 채우려고 리뷰나 소개서에 없는 사실을 새로 만들지 마세요. 특히 메뉴의 맛·특징, 식재료, 조리 과정, 사장님의 운영 철학, 서비스, 매장 분위기, 배달 상황, 손님의 재방문 여부는 리뷰나 소개서에 없으면 언급하지 마세요.\n`
  out += `- 칭찬 리뷰는 공감 → 감사 → 사장님의 자연스러운 반응 순서로 쓰세요.\n`
  out += `- 불편 리뷰는 불편 내용 짚기 → 공감 → 사과 → (필요한 경우) 확인 순서로 쓰세요. 확인되지 않은 원인이나 책임(배달기사·배달 플랫폼·제3자 포함)을 임의로 단정하지 마세요.\n`
  out += `- 칭찬과 불편이 함께 있으면 칭찬에는 감사하고, 불편 사항은 반드시 따로 짚어주세요.\n`
  if (hasAction) {
    out += `- 알려드린 조치만 쓰세요. 조치의 범위를 넓히거나, 아직 하지 않은 조치를 이미 완료한 것처럼 쓰지 마세요.\n`
  } else {
    out += `- 새로운 약속을 만들지 마세요. 금지 예: "앞으로 이런 일이 절대 없도록 하겠습니다", "다음 주문에는 서비스를 넣어드리겠습니다", "바로 직원 교육을 하겠습니다". 확인된 조치가 없으면 "말씀해주신 부분은 확인해보겠습니다" 수준까지만 쓰세요.\n`
  }
  out += `- 구매·재주문·재방문을 과도하게 권유하지 마세요.\n`
  out += `- 리뷰 내용은 이 손님 한 명의 개인적인 경험입니다. 가게의 객관적인 운영 정보나 모든 손님의 공통 평가처럼 일반화하지 마세요.\n`
  ...
  if (hasEventPromo) {
    ...
    out += `- 이번 리뷰와 관계없는 가게 홍보를 덧붙이지 마세요.\n`
  }
  out += `- 전체 말투는 과장 없이 담백하고 정감 있게, 광고대행사가 아니라 사장님이 직접 이야기하는 느낌으로 써주세요.\n`
  if (reviewPlatform === '구글맵') {
    out += `- 구글은 답글을 콘텐츠 정책으로 검토합니다. 정중하고 명확한 표현만 쓰고, 공격적이거나 모호한 표현은 피하세요.\n`
  }

  out += `\n[답글 생성 흐름]\n가능하면 다음 흐름을 따르되, 모든 답글을 똑같은 문장 구조로 만들지는 마세요.\n손님의 구체적인 표현 짚기 → 그 표현에 대한 사장님의 자연스러운 반응 → 감사 또는 공감 → 상황에 맞는 자연스러운 마무리\n`

  out += `\n[출력]\n사장님이 바로 복사해 등록할 수 있는 완성된 리뷰 답글 1개만 보여주세요. "답변 예시", "추천 답글", "배민용 답글" 같은 제목을 붙이지 말고, 작성 이유나 설명도 함께 출력하지 마세요.\n`
...
const PLATFORM_CHAR_LIMITS = [
  ...
  },
  {
    ...
    status: '강의 자료의 등록 화면 캡처로 확인',
    note: '업체를 대표하는 상호명·메뉴명·서비스명 등 손님이 검색할 단어를 등록하는 별도 항목이에요. 상세설명 문장 안에 억지로 나열하지 마세요. 공식 도움말 URL로 재확인하지는 못했으니, 실제 등록 화면의 표시도 함께 확인해주세요.',
    source: null,
  },
  {
